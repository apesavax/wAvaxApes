...
contract AvaxApesMarketV2 is AccessControlEnumerable, Pausable{
    ...
    modifier offerExists(
        ...
        require(
            ...
            "AvaxApesMarket: offer does not exist or expired"
        );
        _;
    }
    ...
    modifier offerNotExists(
        ...
        require(
            offer.quantity == 0 || offer.deadline <= _getNow(),
            "AvaxApesMarket: offer already created"
        );
        _;
    }

    constructor(address AvaxApesAddress, address WrapperAddress, uint devRoyalty, uint minterRoyalty, uint reflectionRoyalty, address admin) {
    ...
    function createPrivateListing(uint256 tokenId, uint256 price, address buyer) external whenNotPaused() isTokenOwner(tokenId) {
        require(masterContract.getApproved(tokenId) == address(this), "AvaxApesMarket: market must be approved to transfer avax ape");
    ...
    function buy(uint256 tokenId) external payable whenNotPaused() {
        ...
        require(masterContract.ownerOf(tokenId) == trade.seller, "AvaxApesMarket: seller must equal current avax ape owner");
        ...
        payable(trade.seller).transfer((msg.value*(100-(_devRoyalty + _minterRoyalty + _reflectionRoyalty))/100));
    ...
    function createOffer(
        ...
        offers[_tokenId][_msgSender()] = Offer(
            1,
            _pricePerItem,
            _deadline
        );
    ...
    function cancelOffer(uint256 _tokenId)
        external
        offerExists(_tokenId, _msgSender()) whenNotPaused()
    {
